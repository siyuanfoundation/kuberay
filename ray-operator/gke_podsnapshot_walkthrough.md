...
* **Gvisor CrashLoopBackOff Deadlock:** Due to the time jump inherent to a PodSnapshot restore phase, the internal Python components of `ray start --block` frequently report severed connections and exit independently. When Kubelet subsequently issues a command to restart the failed sub-container process, `gvisor` blocks it with a native `sandbox is not in started state: state=restored` runtime error, forcing the Head Pod into a permanent `CrashLoopBackOff`. I patched `BuildPod()` inside `pod.go` to intercept `ray.io/gke-podsnapshot-enabled` templates, securely wrapping the generated `rayStartCmd` command block inside an infinite `while true` bash loop natively. Whenever `ray start` panics on a GKE restore, the loop intercepts it and triggers an immediate internal reboot of the Ray container layer without formally yielding an exit code back to the Kubernetes scheduler.
